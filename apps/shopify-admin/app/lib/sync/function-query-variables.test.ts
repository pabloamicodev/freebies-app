...
describe("function config as input-query variables", () => {
  ...
  it("keeps configured attribute keys over the unused placeholders", () => {
    const serialized = JSON.parse(
      serializeFunctionConfig({ offers: [], shippingOffers: [], version: "1", compiledAt: "x", c1: "gift_note", customerTags: ["vip"] } as never),
    ) as Record<string, unknown>;
    expect(serialized).toMatchObject({ c1: "gift_note", c2: "_promo_engine_unused", customerTags: ["vip"] });
  });
});
