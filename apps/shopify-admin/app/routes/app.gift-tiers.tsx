...
import { parseJsonRecord } from "../lib/offer-validation.server.js";
import { normalizeOfferSubconditions } from "../lib/gift-subconditions.js";

export { shopifyHeaders as headers } from "../lib/shopify-headers.js";

export async function action({ request }: ActionFunctionArgs) {
  const [{ db, shopId, currencyCode }, formData] = await Promise.all([
  ...
  try {
    ...
    return {
      success: `Created ${pending.length} draft tier offer(s); ${drafts.length - pending.length} already existed.`,
    };
  } catch (error) {
...
export default function GiftTiersPage() {
  ...
  return (
    <div className="b-page">
      <PageHeader
        title="Gift tiers"
        subtitle="Build exact cumulative or highest-tier-only cart subtotal gifts without changing the storefront design."
      />
