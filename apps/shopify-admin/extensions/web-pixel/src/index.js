...
function flush() {
  ...
  // The pixel runs in a sandboxed worker with no DOM/location — a relative
  // path has no origin to resolve against, so the URL must be absolute.
  if (!shopDomain) return;

  browser.sendBeacon(`https://${shopDomain}${ANALYTICS_ENDPOINT}`, JSON.stringify({
    events: batch,
  }));
}
