...
describe("attribute conditions", () => {
  ...
  it("'exists' passes on any value for a present cart attribute", () => expect(evaluateCartAttribute(cart, { key: "source", matchMode: "exists" }).ok).toBe(true));
  it("'exists' fails when the cart attribute is absent", () => expect(evaluateCartAttribute(cart, { key: "missing", matchMode: "exists" }).ok).toBe(false));
