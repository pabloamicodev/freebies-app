...
class PromoGiftIcon extends HTMLElement {
  ...
  private unsubscribeProductChanged: (() => void) | null = null;
  private countdownTimer: number | null = null;
  private activeOfferId: string = "";
    ...
    this.unsubscribe = on<EvaluationResult>(PromoEvents.EvaluationCompleted, (result) => {
      const qualified = Array.isArray(result.qualifiedOffers)
        ? (this.offerId
            ...
            : result.qualifiedOffers[0])
        : null;
      this.render(qualified ? { offerId: qualified.offerId, offerName: "Free Gift Available" } : null);
    });

    // Also listen for variant changes on product page
    this.unsubscribeProductChanged = on<{ variantId: string }>(
  ...
  disconnectedCallback() {
    this.unsubscribe?.();
    this.unsubscribeProductChanged?.();
    if (this.countdownTimer !== null) window.clearTimeout(this.countdownTimer);
  }

  private render(offer: { offerId: string; offerName: string } | null) {
    if (!this.shadowRoot) return;
    this.activeOfferId = offer?.offerId ?? "";
    ...
    if (offer) {
      this.shadowRoot.querySelector(".pe-gift-icon-wrap")?.addEventListener("click", () => {
        emit(PromoEvents.GiftSliderRequested, { offerId: this.activeOfferId });
        publishAnalytics("promo_engine:widget_clicked", {
          offer_id: this.activeOfferId,
  ...
  private startCountdown(seconds: number) {
    ...
    const update = () => {
      ...
      remaining--;
      if (remaining >= 0) this.countdownTimer = window.setTimeout(update, 1000);
    };
    update();
  }
}

if (!customElements.get("promo-gift-icon")) {
...
class PromoGiftThumbnail extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.unsubscribe = on<EvaluationResult>(PromoEvents.EvaluationCompleted, (result) => {
      ...
      this.activeOfferId = qualified?.offerId ?? "";
      this.render(
        qualified && giftSlider?.offerId === qualified.offerId
          ? giftSlider.selectableGifts
          : null,
      );
    });
  }
  ...
  private render(
    gifts: Array<{ title: string; imageUrl: string | null; variantId: string }> | null,
  ) {
    if (!this.shadowRoot) return;
...
if (!customElements.get("promo-gift-thumbnail")) {
